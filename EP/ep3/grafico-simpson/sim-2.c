...
double erro (double integral) {
    if (integral - 7 < 0)
        return 7 - integral;
    return integral - 7;
}

void imprime (int p, FILE *saida, double integral) {
    fprintf (saida, "%d\t%.16f\n", p, erro (integral));
}

double funcao (double a) {
    return 8 - (5 * pow (a, 4));
}

void simpson (int p, double *integral) {
    long
        j;
    double
        soma_par = 0,
...
/* para partições pares e valores pares para partições impares. */
